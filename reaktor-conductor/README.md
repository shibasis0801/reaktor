...
The desktop has Conversation, Team, Changes, Context and Activity views plus recent-task search. The complete researched destination, six Pencil boards and delivery gates are in [Agent Workspace](https://reaktor.build/docs/reaktor-agent-workspace). Interactive provider input/approval/steering and current Workbench selection context are wired. Isolated parallel coding, editable durable workflows and a fully resolved dependency cone remain pending. Current council stages may continue after a participant failure, which remains visible in the terminal receipt.
